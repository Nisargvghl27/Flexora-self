...
import { LayoutDashboard, Users, Package, FileText, Heart, LogOut, Menu, X } from 'lucide-react';
    ...
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <aside className="hidden md:flex flex-col w-64 bg-gray-900 text-white min-h-screen">
        ...
        <div className="p-4 border-t border-gray-800">
          <button 
            onClick={() => navigate('/')} 
            className="flex items-center gap-3 w-full px-4 py-2 text-gray-400 hover:text-white transition-colors mb-2"
          >
            Go to Main Site
          </button>
          <button 
      ...
      <main className="flex-1 flex flex-col min-w-0">
        ...
        <header className="hidden md:flex bg-white border-b border-gray-200 px-8 py-4 items-center justify-between">
          <h1 className="text-2xl font-bold text-gray-800">{navItems.find(i => i.id === currentTab)?.label}</h1>
          <div className="flex items-center gap-4">
            <span className="text-sm font-medium text-gray-600">Admin: {user?.username}</span>
          </div>
        </header>
